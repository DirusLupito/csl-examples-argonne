...
def main():
  ...
  for size_idx, size in enumerate(size_values):
    ...
    u_exact_values = np.sin(np.pi*X) * np.sin(np.pi*Y) * np.sin(np.pi*Z)
    
    # Compute the source term (right-hand side)
    source_term_values = -3 * np.pi**2 * np.sin(np.pi*X) * np.sin(np.pi*Y) * np.sin(np.pi*Z)
    ...
    for i in range(height):
        for j in range(width):
            for k in range(pe_length):
                if (i == 0 or i == height-1 or j == 0 or j == width-1 or k == 0 or k == pe_length-1):
    ...
    for i in range(height):
      for j in range(width):
        stencil_coeff[(i, j, 0)] = 1.0 / (h*h)  # west
        stencil_coeff[(i, j, 1)] = 1.0 / (h*h)  # east
        stencil_coeff[(i, j, 2)] = 1.0 / (h*h)  # south
        stencil_coeff[(i, j, 3)] = 1.0 / (h*h)  # north
        stencil_coeff[(i, j, 4)] = 1.0 / (h*h)  # bottom
        stencil_coeff[(i, j, 5)] = 1.0 / (h*h)  # top
        stencil_coeff[(i, j, 6)] = -6.0 / (h*h)  # center
